...
<body>
	<div class="barnavtop">您所在的位置：留言管理</div>
	<div id="workspace">
		...
		<div
			...
			<div style="text-align: right; height: 30px; margin-top: 10px">
				<span style="float: left; font-size: 14px; font-weight: bold">留言管理
				</span>
			</div>
				...
				<table id="CmsSiteList_table" class="tableRegion" width="100%"
					...
					<thead>
						<tr>
							...
							<td width="17%" class="tableHeader" title="留言者"><a href="javascript:void(0)"
								id="t_messager">留言者</a></td>
								
							<td width="17%" class="tableHeader" title="地址"><a
								href="javascript:void(0)" id="t_address">地址</a></td>
								
							<td width="17%" class="tableHeader" title="电话"><a
								href="javascript:void(0)" id="t_tel">电话</a></td>
								
							<td width="17%" class="tableHeader" title="email"><a
								href="javascript:void(0)" id="t_email">email</a></td>
								
							<td width="17%" class="tableHeader" title="留言时间"><a
								href="javascript:void(0)" id="t_messageTime">留言时间</a></td>
						...
						<tr>
							<form action="listTrends.do" name="queryForm" id="queryForm"
								...
								<input type="text"
									value="${param.messager}" name="source" id="source"  class="x_inputjing"/>
								</td>
								
								<td align="center" class="statusBar">
								<input type="text"
									value="${param.address}" name="source" id="source"  class="x_inputjing"/>
								</td>
								
								<td align="center" class="statusBar">
								<input type="text"
									value="${param.tel}" name="source" id="source"  class="x_inputjing"/>
								</td>
								
								<td align="center" class="statusBar">
								<input type="text"
									value="${param.email}" name="source" id="source"  class="x_inputjing"/>
								</td>
								
								<td align="center" class="statusBar">
								<input type="text"
									value="${param.messageTime}" name="publishTime" id="publishTime"  class="x_inputjing" />
								</td>
					...
					<form action="">
						<c:forEach items="${result}" var="item" varStatus="status">
							<tr class="odd" onmouseover="this.className='highlight'"
								...
								<td align="center">${item.messager}</td>
								<td align="center">${item.address}</td>
								<td align="center">${item.tel}</td>
								<td align="center">${item.email}</td>
								<td align="center">
									<fmt:formatDate value="${item.messageTime}" pattern="yyyy-MM-dd HH:mm:ss" />
								</td>
								...
								<a class="sexybutton"
								 ...
								 <a class="sexybutton"
									href="messageDetail.do?id=${item.id}">
